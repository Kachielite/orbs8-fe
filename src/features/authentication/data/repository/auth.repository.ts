...
export class AuthRepositoryImpl implements AuthRepository {
    ...
    async register(payload: RegisterSchemaType): Promise<Either<Failure, string>> {
        try {
    ...
    async refreshToken(payload: RefreshTokenSchemaType): Promise<Either<Failure, AuthModel>> {
        try {
    ...
    async loginWithGoogle(payload: LoginWithGoogleSchemaType): Promise<Either<Failure, string>> {
        try {
    ...
    async requestPasswordReset(payload: RequestPasswordResetSchemaType): Promise<Either<Failure, string>> {
        try {
            const response = await this.authDataSource.requestPasswordReset(payload);
            return right(response);
        } catch (error) {
            throw extractErrorRepository(error, 'AuthRepositoryImpl:RequestPasswordReset');
        }
    }

    async resetPassword(payload: ResetPasswordSchemaType): Promise<Either<Failure, string>> {
        try {
