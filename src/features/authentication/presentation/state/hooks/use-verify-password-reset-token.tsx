...
const useVerifyPasswordResetToken = () => {
  ...
  const {isLoading: isVerifyingPassword} = useQuery(
      ...
      async () => {
          if (!token || !email) {
              throw new Failure('Invalid reset password link, please request a new one')
            }
      ...
      },
      {
        onSuccess: () => {
          ...
          toast.success('Reset link verified successfully, you can reset your password now');
          navigate('/login');
        },
        onError: error => {
          const errorMessage = extractErrorHooks(error, 'useVerifyPasswordResetToken');
