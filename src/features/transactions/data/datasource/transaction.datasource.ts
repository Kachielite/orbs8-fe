...
} from '@/features/transactions/data/model/transaction.model';
import {
  ITransactionQuery,
...
export class TransactionDataSource implements ITransactionDatasource {
  ...
  constructor(
    @inject(TransactionNetwork) transactionNetwork: TransactionNetwork
  ) {
  ...
  async getTransactionById(id: number): Promise<TransactionModel> {
    try {
      ...
      throw extractErrorDatasource(
        error,
        'TransactionDataSource:getTransactionById'
      );
    }
  }
  ...
  async getTransactionSummary(
    ...
    try {
      const response =
        await this.transactionNetwork.getTransactionSummary(query);
      return TransactionSummaryModel.fromJSON(response);
    } catch (error) {
      throw extractErrorDatasource(
        error,
        'TransactionDataSource:getTransactionSummary'
      );
    }
  }
}
