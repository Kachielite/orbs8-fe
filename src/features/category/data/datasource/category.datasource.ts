...
export class CategoryDataSource implements ICategoryDataSource {
    constructor(@inject(CategoryNetwork) private readonly categoryNetwork: CategoryNetwork) {}

    async getCategories(): Promise<CategoryModel[]> {
        try {
            const response = await this.categoryNetwork.getCategories();
            return response.map((category: CategoryModel) => CategoryModel.fromJSON(category));
        } catch (error) {
