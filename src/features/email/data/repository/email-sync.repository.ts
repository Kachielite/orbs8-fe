...
export class EmailSyncRepository implements IEmailSyncRepository {
    constructor(
        @inject('IEmailDatasource') private readonly emailDatasource: IEmailSyncDataSource
    ) {}
    ...
    async getToken(request: GetOauthTokenSchemaType): Promise<Either<Failure, string>> {
        try {
