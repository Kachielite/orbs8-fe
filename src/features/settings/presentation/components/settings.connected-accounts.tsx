...
export const SettingsConnectedAccountItem = () => {
    ...
    const {emailLabelForm, verifyingAccess, verifyEmailLabelAccessHandler} =
        useVerifyEmailLabel();
    useGetSyncStatus();

    const [showDetails, setShowDetails] = React.useState(false);
    const [showDeleteDataDialogue, setShowDeleteDataDialogue] =
    ...
    return (
        <div className="flex flex-col gap-6 w-full border border-muted-foreground rounded-md px-4 py-3">
            ...
            {showDetails &&
                (user?.emailLinked ? (
                    <div className="flex flex-col w-full gap-7">
                        <div className="flex flex-row items-start gap-4">
                            <div className="w-[90%] flex flex-col gap-2">
                                ...
                                <p className="text-muted-foreground text-sm">
                                    This is the email label that is being sync. Update value if
                            ...
                            <button
                                ...
                                onClick={() =>
                                    verifyEmailLabelAccessHandler({
                                        labelName: emailLabelForm.getValues('labelName'),
                                    })
                                }
                        ...
                        <button
                            onClick={() => setShowDeleteDataDialogue(true)}
                    ...
                    <div className="flex flex-col items-center gap-4">
                        <p className="text-sm text-foreground">
                            Connect your Gmail account to start syncing your emails. Once
                            connected, you can manage your email label and data from here.
                        </p>
                        <button
            ...
            <SettingsDeleteData
                setVisibility={setShowDeleteDataDialogue}
                visibility={showDeleteDataDialogue}
            />
        </div>
    );
};
